...
def get_gradients(model, img_input, top_pred_idx):
    ...
    Returns:
        Gradients of the predictions w.r.t. image_input
    """
...
def get_integrated_gradients(model, img_input, top_pred_idx, baseline=None, num_steps=50):
    """ Computes Integreated Gradients for a predicted label.

    Args:
        img_input (ndarray>? modified): Original image
        top_pred_idx: Predicted label for the input image
        baseline (ndarray>? modified): The baseline image to start with for interpolation
        num_steps: Number of interpoloation steps between the baseline
    ...
    # 2. Preporcess the interpolated images
...
def random_baeline_integrated_gradients(
    ...
    Args:
        img_input: 3D image
        top_pred_idx: Predicted label for the input image
        num_stpes: Number of interpolation steps between the baseline
            and the input used in the computation of integrated gradients.
            These steps along determine the integral approximation error.
            By default, num_steps is set to 50
        num_runs: number of baseline images to generate

    Returns:
        Averaged integrated gradients for 'num_runs' baseline imaged
    """

    # 1. List ot keep track of Integrated Gradient (IG) for all the images
    integrated_grads = []

    # 2. Get the integrated gradients for all the baseline
...
class GradVisualizer:
    ...
    def apply_linear_transformation(self,
        ...
        # 2. Transform the attributinos by a linear function f(x) = a*x+b
        # such that f(m_ = 1.0 and f(e) = lower_end
        transformed_attributions = (1-lower_end)*(np.abs(attributions)-e)/(m-e)+lower_end
        #transformed_attributions = attributions

        # 3. Make sure that the sign of transformed attributinos is the same as original attributions
    ...
    def draw_outlines(self,
                      attributions, percentage=90, connected_component_structure=np.ones((3,3))):
    ...
    def visualize(
        ...
        ax[1].imshow(grads_attr)
        ax[2].imshow(igrads_attr)
