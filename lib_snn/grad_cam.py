...
def make_gradcam_heatmap_snn(img_array, model, last_conv_layer_name, neuron_mode, pred_index=None):
    ...
    # fwd model for dy_dx - y: a hidden neuron, x:input pixel
    # gradient propagation in fwd path to get dy_dx
    grad_model_dy_dx = lib_snn.model.Model(
        model.inputs, model.get_layer(last_conv_layer_name).output, batch_size, input_shape,  classes, conf
    )
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        #tape.watch(grad_model.get_layer('conv1')._outputs)
        #tape.watch(grad_model.get_layer('conv2')._outputs)
        last_conv_layer_output, preds = grad_model(img_array)
        if pred_index is None:
            pred_index = tf.argmax(preds[0])
        class_channel = preds[:,pred_index]
    ...
    if neuron_mode:
        if True:
            ...
            # norm grad
            if norm_grad:
                g3 = g3/tf.reduce_max(g3,axis=[1,2,3],keepdims=True)
            ...
            if not mean_t:
                ...
                if spike_count_norm:
                    sc = tf.reduce_sum([a0,a1,a2,a3])
                    a = tf.math.divide_no_nan(a,sc)
                    g = tf.math.divide_no_nan(g,sc)
                else:
                    a = a / 4
                    g = g / 4
...
# accumulated spike version
def make_gradcam_heatmap_snn_old(img_array, model, last_conv_layer_name, neuron_mode, pred_index=None):
