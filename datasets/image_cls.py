...
def load(dataset_name,batch_size,input_size,input_size_pre_crop_ratio,num_class,train,num_parallel,conf,input_prec_mode,preprocessor_input):
    ...
    if train:
        ...
        if conf.calibration_idx_test or conf.vth_search_idx_test or (not conf.mode=='train'):
            train_ds, train_ds_info = tfds.load(dataset_name, split=data_label_train, shuffle_files=False, as_supervised=True, with_info=True,
            ...
            train_ds, train_ds_info = tfds.load(dataset_name, split=data_label_train, shuffle_files=True, as_supervised=True, with_info=True,
                                                data_dir = data_dir, download_and_prepare_kwargs = download_and_prepare_kwargs, )
    ...
    if train:
        if conf.data_aug_mix == 'mixup':
            train_ds = train_ds.map(lambda train_ds_1, train_ds_2: mixup(train_ds_1, train_ds_2, 1.0, input_prec_mode,preprocessor_input),
