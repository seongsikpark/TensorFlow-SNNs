...
# output_node = akc.ConvBlock(num_blocks=1, num_layers=1, separable=False, max_pooling=False, tunable=True)(output_node)
# output_node = akc.ConvBlock(num_blocks=1, num_layers=1, separable=False, max_pooling=False, tunable=True)(output_node)
# output_node = akc.ConvBlock(num_blocks=1, num_layers=3, separable=False, max_pooling=True, tunable=True)(output_node)
# output_node = akc.ConvBlock(num_blocks=1, num_layers=3, separable=False, max_pooling=False, tunable=True)(output_node)
# output_node = akc.ConvBlock(num_blocks=1, num_layers=1, separable=False, max_pooling=True, tunable=True)(output_node)
...
output_node = akc.DenseBlock(num_layers=1, use_batchnorm=True, tunable=True)(output_node)
output_node = akc.ClassificationHead(num_classes=num_class, dropout=0, loss=loss, metrics=metrics, tunable=True)(output_node)
...
# cb_lib_snn = lib_snn.callbacks.SNNLIB(config.flags, config.filepath_load, train_ds_num, test_ds_num)
cb_lib_snn = lib_snn.callbacks.SNNLIB(config.flags, model_path, train_ds_num, test_ds_num)
...
# show the best trial at the end of each trial
...
if config.flags.nn_mode == 'ANN':
    tf.keras.models.Model.save(self=model, filepath=model_path, save_format="tf")
    print("@@DONE1_DNN@@")
else:
