...
if conf.nn_mode=='SNN' and conf.dnn_to_snn:
    ...
    model = models.utils.transfrom_model_from_v1(model,load_weight,model_dataset_name)
...
if train:
    ...
    # save model
    file_name = load_weight.split('/')[-1]
    path_model_save_root='/home/sspark/Models/DNN/CNN'
    path_model_save=os.path.join(path_model_save_root,model_dataset_name)
    file_path_model_save=os.path.join(path_model_save,file_name)

    os.makedirs(paht_model_save, exist_ok=True)

    model.save_weights(file_path_model_save)
