...
def mlp(x, in_features,k_init,tdbn,dropout_rate=0, name_num=0, hidden_features=None, out_features=None):
    ...
    x = lib_snn.layers.BatchNormalization(en_tdbn=tdbn, fused=True,name='MLP_bn_fc1_' + str(name_num))(x)
    ...
    x = lib_snn.layers.BatchNormalization(en_tdbn=tdbn,fused=True, name='MLP_bn_fc2_' + str(name_num))(x)
...
def ssa(x, dim,k_init,tdbn, num_heads=12, name_num=0, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
    ...
    q_b = lib_snn.layers.BatchNormalization(en_tdbn=tdbn,fused=True, name='q_bn' + str(name_num))(q_d)
    ...
    k_b = lib_snn.layers.BatchNormalization(en_tdbn=tdbn, fused=True,name='k_bn' + str(name_num))(k_d)
    ...
    v_b= lib_snn.layers.BatchNormalization(en_tdbn=tdbn, fused=True,name='v_bn' + str(name_num))(v_d)
    ...
    # attn = tf.keras.layers.Lambda(lambda tensors: tf.einsum('bnqd,bnkd->bnqk',tensors[0],tensors[1]))([q,k])
    # attn = tf.keras.layers.EinsumDense('bnqd,bhkd->bhqk',output_shape=(12,64,64),kernel_initializer='glorot_uniform',name='ssa_qk'+str(name_num))([q,k])
    # attn = attn*scale
    ...
    # x = tf.keras.layers.EinsumDense('bnqk,bhvd->bhqd',output_shape=(12,64,32),kernel_initializer='glorot_uniform',name='ssa_qkv'+str(name_num))([attn,v])
    # x = tf.keras.layers.Lambda(lambda tensors: tf.einsum('bnqk,bnvd->bnqd',tensors[0],tensors[1]))([attn,v])
    x = tf.keras.layers.Lambda(lambda tensors: tf.matmul(tensors[0],tensors[1]))([attn,v])
    # x = x(tf.keras.Input(shape=(12,64,32)))
    # sspark
    ...
    x = lib_snn.layers.BatchNormalization(en_tdbn=tdbn, fused=True,name='proj_bn' + str(name_num))(x)
...
def sps(x, input_shape,k_init,tdbn,patch_size=4, embed_dims=384):
    ...
    norm_c1 = lib_snn.layers.BatchNormalization(en_tdbn=tdbn,fused=True, name='bn_conv1')(syn_c1)
    ...
    norm_c2 = lib_snn.layers.BatchNormalization(en_tdbn=tdbn, fused=True,name='sps_bn2')(syn_c2)
    ...
    bn_c3 = lib_snn.layers.BatchNormalization(en_tdbn=tdbn,fused=True, name='sps_bn3')(syn_c3)
    ...
    bn_c4 = lib_snn.layers.BatchNormalization(en_tdbn=tdbn, fused=True,name='sps_bn4')(syn_c4)
    ...
    rpe_norm_c1 = lib_snn.layers.BatchNormalization(en_tdbn=tdbn, fused=True,name='sps_bn_rpe')(rpe_c1)
...
def sps_ImageNet(x, input_shape,k_init,tdbn,patch_size=4, embed_dims=384):
    ...
    norm_c1 = lib_snn.layers.BatchNormalization(en_tdbn=tdbn,fused=True, name='sps_bn1')(syn_c1)
