...
def ssa(x, dim,k_init,tdbn, num_heads=12, name_num=0, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
    ...
    q = tf.keras.layers.Reshape(( 64, 12,32))(q_linear_out)
    ...
    k = tf.keras.layers.Reshape(( 64, 12,32))(k_linear_out)
    # k_ = tf.keras.layers.Reshape(( 64, 12,32))(k_linear_out)
    ...
    v = tf.keras.layers.Reshape(( 64, 12,32))(v_linear_out)
    # v = tf.reshape(v_linear_out, [B, N, num_heads, C // num_heads])
    # v = tf.transpose(v, perm=[0, 2, 1, 3])
    attn = tf.keras.layers.Lambda(lambda tensors: tf.matmul(tensors[0],tensors[1]))([q,tf.keras.layers.Reshape((64,32,12))(k)])
    x = tf.keras.layers.Lambda(lambda tensors: tf.matmul(tensors[0],tensors[1]))([attn,v])

    # attn = tf.matmul(q, k, transpose_b=True) * scale
    # x = tf.matmul(attn, v)

    x = tf.keras.layers.Reshape(( 64, 384))(x)
    # x = tf.transpose(x, perm=[0, 1, 2, 3 ])  # [T, B, N, num_heads, head_dim]
    # x = tf.reshape(x, [100, 64, 384])  # Combine head_dim and num_heads
...
def block(x, dim, num_heads,k_init,tdbn, name_num=0, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
    ...
    block_in = x
    attn_out = ssa(norm1(x), dim=dim,k_init=k_init,tdbn=tdbn, num_heads=num_heads, qkv_bias=qkv_bias,
    ...
    mlp_out = mlp(norm2(x), name_num=name_num,k_init=k_init,tdbn=tdbn, in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim)
...
def sps(x, input_shape,k_init,tdbn,tdbn_first_layer,use_bn_feat, patch_size=4, embed_dims=384):
    ...
    x = tf.keras.layers.Reshape((64,embed_dims))(rpe_out)
...
def spikformer(
    ...
    # gap_x = tf.keras.layers.GlobalAveragePooling1D(data_format=data_format)(gap_x_f)
    output_tensor = lib_snn.layers.Dense(classes, last_layer=True, kernel_initializer=k_init,name='predictions')(x_f)
