...
def ssa(x, dim,k_init,tdbn, num_heads=12, name_num=0, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
    ...
    #q= lib_snn.layers.Lambda(lambda x : tf.transpose(x, perm=[0,2,1,3]), temporal_batch=True)(q)    # [B,head,N,dim/head]
    #q= lib_snn.layers.Lambda(lambda x : tf.transpose(x, perm=[0,2,1,3]))(q)    # [B,head,N,dim/head]
    #q= tf.keras.layers.Permute((2,1,3))(q)    # [B,head,N,dim/head]
    q= lib_snn.layers.Permute((2,1,3), temporal_batch=True)(q)    # [B,head,N,dim/head]
    ...
    k= lib_snn.layers.Permute((2,1,3), temporal_batch=True)(k)    # [B,head,N,dim/head]
    ...
    v= lib_snn.layers.Permute((2,1,3), temporal_batch=True)(v)    # [B,head,N,dim/head]
    ...
    attn = lib_snn.layers.Lambda(lambda tensors: tf.matmul(tensors[0],tensors[1]))([q,kt])
    #attn = tf.keras.layers.Lambda(lambda x: x*scale)(attn)
    attn = lib_snn.layers.Lambda(lambda x: x*scale, temporal_batch=True)(attn)
    #attn = attn*scale
