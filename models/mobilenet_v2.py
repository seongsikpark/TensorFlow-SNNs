...
def MobileNetV2(batch_size,
    ...
    # Conv1
    x = lib_snn.layers.Conv2D(first_block_filters, kernel_size=3, strides=(2,2), padding='same', use_bn=True, activation='relu', relu_max_value=6, name='Conv1')(img_input)
    ...
    x = lib_snn.layers.Conv2D(last_block_filters, kernel_size=1, use_bn=True, activation='relu', relu_max_value=6, name='Conv_1')(x)
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id):
    ...
    if block_id:
        ...
        n_channels = expansion * in_channels
        x = lib_snn.layers.Conv2D(n_channels, kernel_size=1, padding='same', use_bn=True, activation='relu', relu_max_value=6, name=prefix+'expand')(x)
    else:
    ...
    x = lib_snn.layers.Conv2D(pointwise_filters, kernel_size=1, padding='same', use_bn=True, activation=None, name=prefix + 'project')(x)

    if in_channels == pointwise_filters and stride == 1:
        #return layers.Add(name=prefix + 'add')([inputs, x])
        return lib_snn.layers.Add(use_bn=False, name=prefix+ 'add')([inputs, x])
